        ...
        <div class="flex h-16 items-center justify-between">
            ...
            <div class="hidden md:block">
                <div class="ml-4 flex items-center md:ml-6">
                            ...
                            <div class="relative">
                                ...
                                <label for="dropdown-toggle" class="rounded cursor-pointer">
                                    ...
                                    <img class="h-8 w-8 object-cover rounded-full"
                                        src="<?= (new Core\User)->getFolderByEmail($_SESSION['user']['email']) . $image ?>"
                                        alt="">
                                    <?php else :
                                        ...
                                        ?>
                                    <p
                                        class="h-8 w-8 flex justify-center items-center border border-white rounded-full p-6 text-xl font-extrabold text-white">
                                        <?= $letter ?>
                                    </p>
                                    <?php endif; ?>
                                </label>
                    ...
                    <div class="ml-3">
                        <a href="/register"
                            class="<?= urlIs('/register') ? 'bg-gray-900 text-white' : 'text-gray-300' ?> hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Регистрация</a>
                        <a href="/login"
                            class="<?= urlIs('/login') ? 'bg-gray-900 text-white' : 'text-gray-300' ?> hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Вход</a>
                    </div>
                    <?php endif ?>
                </div>
            </div>
        ...
        <div class="border-t border-gray-700 pt-4 pb-3">
            <div class="flex items-center px-5">
                <div class="flex-shrink-0">
                    ...
                    <img class="h-8 w-8 object-cover rounded-full"
                        src="<?= (new Core\User)->getFolderByEmail($_SESSION['user']['email']) . $image ?>" alt="">
                    <?php endif; ?>
                </div>
                <div class="ml-3">
                    <?php if (isset($_SESSION['user'])) : ?>
            ...
            <div class="mt-3 space-y-1 px-2">
                <a href="/profile"
                    class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">Ваш
                    профиль</a>
